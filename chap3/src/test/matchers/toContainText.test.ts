...
import { stripTerminalColor } from './utils/stripTerminalColor'
...
describe('toContainText matcher', () => {
  ...
  it('returns a message that contains the source line if no match', () => {
    ...
    const {message} = toContainText(domElement, 'text to find')
    
    expect(stripTerminalColor(message())).toContain(`expect(element).toContainText("text to find")`)
  })
